function [z,E] = run_dpgmm_flow(data,start,dispOn,Mproc,sc,as,alpha, endtime, numits)
...
for it=1:numits
    ...
    if (dispOn)
        ...
        for z=min(tz(:)):max(tz(:))
            ...
            hold on;
            if (any(mask))
                error_ellipse(clusters(k).Sigma_l(1:2,1:2), clusters(k).mu_l(1:2), 'color', [1 1 1],'Linewidth',3);
                error_ellipse(clusters(k).Sigma_l(1:2,1:2), clusters(k).mu_l(1:2), 'style','--','color', colors(z+1,:),'Linewidth',1);
            end
            mask = tz==z & ~left;
            plot(data(1,mask), data(2,mask), 'o', 'Color', colors(z+1,:));
            hold on;
            if (any(mask))
                error_ellipse(clusters(k).Sigma_r(1:2,1:2), clusters(k).mu_r(1:2), 'color', [1 1 1],'Linewidth',3);
                error_ellipse(clusters(k).Sigma_r(1:2,1:2), clusters(k).mu_r(1:2), 'style','--','color', colors(z+1,:),'Linewidth',1);
            end
        end
        drawnow;
    end
